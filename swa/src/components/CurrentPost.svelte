...
async function toggleFavorite() {
    ...
    if (isFavorited) {
        ...
        await updateDoc(userDocRef, {
            favoritePosts: arrayRemove(currentpostId)
        });
    } else {
        // Aggiungi ai preferiti
        await updateDoc(userDocRef, {
            favoritePosts: arrayUnion(currentpostId)
        });
    }
...
onMount(async () => {
    ...
    if (auth.currentUser) {
        ...
        if (userData && userData.favoritePosts && userData.favoritePosts.includes(currentpostId)) {
            isFavorited = true;
        }
    }
});
...
<div class="current-post-container">
    <h2 class="current-post-title"><a href={`/post/${currentpostId}`} class="post-link">{title}</a></h2>
    <p>{content}</p>
    <div class="current-post-meta">
        <span>Written by {author} on {date}</span>
        <span class="current-post-tag">Tag: {tag}</span>
    </div>
    <button class="current-favorite-button" on:click={toggleFavorite}>
        {#if isFavorited}
            🌟  Rimuovi dai Preferiti
        {:else}
            ⭐ Aggiungi ai Preferiti
    ...
    <div class="actions-container">
        ...
        <Upvote initialUpvotes={initialUpvotes} postId={currentpostId} />
    </div>
    {#if auth.currentUser && auth.currentUser.uid === authorId}
        <button class="current-delete-button"on:click={deletePost}>🗑 Elimina</button>
    {/if}
</div>
